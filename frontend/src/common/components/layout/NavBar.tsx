...
export type TabKey = 'home' | 'simulados' | 'desempenho' | 'ranking' | 'ajustes'
      ...
      >
        {items.map((item) => {
          ...
          return (
            ...
            >
              <Ionicons name={focused ? item.icon : `${item.icon}-outline` as any} size={24} color={color} />
              <Text style={{ color, fontSize: 12, marginTop: 4 }}>{item.label}</Text>
            </TouchableOpacity>
          )
        })}
      </View>
        ...
        {items.map((item) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => onPress(item.key)}
            >
              <Ionicons name={focused ? item.icon : `${item.icon}-outline` as any} size={20} color={color} />
